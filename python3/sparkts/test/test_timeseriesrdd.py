...
class TimeSeriesSerializerTestCase(TestCase):
    def test_times_series_serializer(self):
        ...
        self.assertEqual(reconstituted[0][0], series[0][0])
        self.assertEqual(reconstituted[1][0], series[1][0])
...
class TimeSeriesRDDTestCase(PySparkTestCase):
    def test_time_series_rdd(self):
        ...
        self.assertEqual(tsrdd.count(), 3)
        ...
        self.assertEqual(subslice.index(), uniform('2015-04-10', periods=6, freq=freq, sc=self.sc))
    ...
    def test_to_instants(self):
        ...
        rdd = self.sc.parallelize(list(zip(labels, vecs)), 3)
        tsrdd = TimeSeriesRDD(dt_index, rdd)
        samples = tsrdd.to_instants().collect()
        target_dates = ['2015-4-9', '2015-4-10', '2015-4-11', '2015-4-12']
        self.assertEqual([x[0] for x in samples], [pd.Timestamp(x) for x in target_dates])
    ...
    def test_to_pandas_series_rdd(self):
        ...
        self.assertEqual(len(vecs), len(series_arr))
        for i in range(len(vecs)):
            self.assertEqual(series_arr[i][0], labels[i])
